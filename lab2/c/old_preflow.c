...
// Structure to pass arguments to the push thread
typedef struct {
    graph_t* g;
    node_t* u;
    node_t* v;
...
static void* push(void* args)
{
	pr("in push\n");
	push_args_t* push_args = (push_args_t*) args;

    // Use the unpacked arguments to perform the push operation
    graph_t* g = push_args->g;
    node_t* u = push_args->u;
    node_t* v = push_args->v;
...
// Function to safely access the alive_threads_counter using a mutex
int get_alive_threads(int* alive_threads, pthread_mutex_t* alive_threads_lock) {
    int value;
...
int preflow(graph_t* g)
{
	...
	pthread_t* threads = NULL;  // Pointer to array of threads
    int thread_count = 0;    // Number of threads created
    int max_threads = 10;    // Adjust size as needed; consider dynamically resizing if necessary
	
	int alive_threads_counter = 0;
	pthread_mutex_t alive_threads_lock;
	pthread_mutex_init(&alive_threads_lock, NULL);
	...
	while (p != NULL) {
        ...
        if (thread_count >= max_threads) {
			...
			pr("STARTLOOP: reallocated threads to %d threads\n", max_threads);
        }
		e = p->edge;
		p = p->next;
		...
		push_args->g = g;
		push_args->u = s;
		push_args->v = other(s, e);  // Assuming other(s, e) is a function returning a node_t*
		push_args->e = e;
		push_args->thread_counter = &alive_threads_counter;
		push_args->thread_counter_lock = &alive_threads_lock;
		push_args->condThreadContinue = &condThreadContinue;

		//Create thread
		pr("Starting thread in start\n");
		add_alive_threads(&alive_threads_counter, &alive_threads_lock);
		int result = pthread_create(&threads[thread_count], NULL, push, push_args);
		if (result != 0) {
			// Handle thread creation failure
			perror("pthread_create failed");
			free(push_args);  // Free memory if thread creation fails
			exit(1);
    	}
		thread_count++;
		// push(push_args);
	...
	free(threads);
	pthread_mutex_destroy(&alive_threads_lock);
	pr("START LOOP OVER\n");
	print_graph(g);
	...
	pthread_mutex_init(&main_alive_threads_lock, NULL);


	/* then loop until only s and/or t have excess preflow. */
	// terminatare program when no_excess AND no threads working
	// 	while ((u = leave_excess(g)) != NULL || alive_threads != 0) {
	pthread_cond_t condmainThreadContinue;
	pthread_cond_init(&condmainThreadContinue, NULL);
	pthread_mutex_lock(&main_alive_threads_lock);
	// int dbg_t;
	time_t start_time = time(NULL); // Record the start time
	const time_t time_limit = 10; // Set the time limit to 30 seconds

	while ((u = leave_excess(g)) != NULL || (main_alive_threads_counter > 0)){
	// while ((u = leave_excess(g)) != NULL || (dbg_t = get_alive_threads(&main_alive_threads_counter, &main_alive_threads_lock)) != 0) {
		// if (difftime(time(NULL), start_time) >= time_limit) {
		// 	printf("Time limit exceeded. Exiting loop.\n");
		// 	break; // Exit the loop if 30 seconds have passed
    	// }
		pr("gbn1\n");
			if (main_alive_threads_counter != 0){
				pr("still have alive threads: %d\n", main_alive_threads_counter);
			}
			if (u == NULL && main_alive_threads_counter == 0){
				pr("WTF: %d\n", main_alive_threads_counter);
				break;
			}
		...
		while (u != NULL){
			...
			*/
			...
			if (v != NULL) {
				...
				main_push_args->v = v;
				main_push_args->e = e;
				main_push_args->thread_counter = &main_alive_threads_counter;
				main_push_args->thread_counter_lock = &main_alive_threads_lock;
				main_push_args->condThreadContinue = &condmainThreadContinue;
				// push(push_args);
		...
		pthread_mutex_lock(&main_alive_threads_lock);
		// while ((u = leave_excess(g)) != NULL || main_alive_threads_counter > 0)
		// while ((u = leave_excess(g)) == NULL && main_alive_threads_counter <= 0) //This no good, but anyways..
		while (!excess_left(g)) {
			if (main_alive_threads_counter == 0){
				pr("PROGRAM SHOULD BE DONE...\n");
				break;
			}
			pr("WAIT FOR COND u: %p n_threads: %d\n", u, main_alive_threads_counter);
			pthread_cond_wait(&condmainThreadContinue, &main_alive_threads_lock);
			pr("GOT COND SIGNAL\n");
		}
		pr("LOOP PREFORMED\n");
		// pthread_mutex_unlock(&main_alive_threads_lock);
